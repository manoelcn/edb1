...
double maiorMediaMovel(const int numeros[], int tamanho) {
    if (tamanho < 4) {
        throw std::invalid_argument("array com menos de 4 elementos.");
    }
    int janelas = tamanho - 3;
    double mediaFinal = (numeros[0] + numeros[1] + numeros[2] + numeros[3]) / 4.0;
    for (int j = 0; j < janelas; j++) {
        double mediaAtual = 0;
        mediaAtual = (numeros[j] + numeros[j + 1] + numeros[j + 2] + numeros[j + 3]) / 4.0;
        if (mediaAtual > mediaFinal) {
            mediaFinal = mediaAtual;
        }
    }
    return mediaFinal;
}
